<template>
  <div>
    <el-dropdown class="head-button" @command="handleDropdownCommand">
      ...
      >
        ...
        <el-form-item label="文件类型" prop="bookType">
          <el-radio-group v-model="exportExcel.bookType" class="formRadio">
            <el-radio-button label="xlsx"><svg-icon icon-class="xlsx" class-name="exportSvg" /></el-radio-button>
            <el-radio-button label="csv"><svg-icon icon-class="csv" class-name="exportSvg" /></el-radio-button>
            <el-radio-button label="txt"><svg-icon icon-class="txt" class-name="exportSvg" /></el-radio-button>
          </el-radio-group>
        </el-form-item>
        <el-form-item class="from-footer">
          <el-button type="primary" :disabled="exportTrue" @click="getData('export')">下载</el-button>
...
export default {
  ...
  mounted() {
    this.debounceSubmitForm = this._.debounce(this.submitForm, 1000)
  },
  methods: {
    ...
    getData(formName) {
      ...
      this.$emit('change', this.selected)
      this.debounceSubmitForm(formName)
    },
